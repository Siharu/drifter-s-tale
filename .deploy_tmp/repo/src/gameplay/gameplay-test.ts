...
import { WeatherType, ItemID, BuildingID } from '../types.js';
...
function createTestDrifter(zone: ReturnType<typeof createTestWorld>) {
  const roster = new DrifterRoster(12345);
  const drifterEntity = roster.drawDrifter('test-run' as any, zone.id, { x: 128, y: 128 });
...
function runGameplaySmokeTest() {
  ...
  const item = {
    ...
    iconIndex: 1,
    isWeapon: false as const,
    value: 5,
    weight: 1,
    position: { x: drifter.position.x, y: drifter.position.y },
    buildingID: ID.create<BuildingID>('building'),
    roomID: null,
  };
