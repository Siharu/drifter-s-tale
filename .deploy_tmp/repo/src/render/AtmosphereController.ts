...
export interface AtmosphereState {
  ...
  obsediaRainIntensity: number; // 0–1
  decayLevel: number; // 0–1 — zone decay, drives vignette strength independent of wrongness
}

interface VignettePreset {
  color: string;     // rgba string, alpha pre-baked low — multiplied by decay/strength at apply time
  strength: number;  // 0–1 base strength before decayLevel modulation
}
...
const VIGNETTE_PRESETS: Record<WrongnessState, VignettePreset> = {
...
const RAIN_WEATHER: ReadonlySet<WeatherType> = new Set(['ACID_RAIN'] as WeatherType[]);
const RAIN_WRONGNESS: ReadonlySet<WrongnessState> = new Set([WrongnessState.RAINY, WrongnessState.STORMY]);

export class AtmosphereController {
  private container: HTMLElement;
  private root: HTMLDivElement;
  private vignetteEl: HTMLDivElement;
  private rainEl: HTMLDivElement;
  private obsediaEl: HTMLDivElement;
  private glitchEl: HTMLDivElement;
  private styleEl: HTMLStyleElement | null = null;

  private glitchTimer: number = 0;
  private nextGlitchAt: number = 0;
  ...
  private injectStyles(): void {
    if (document.getElementById('drifter-atmosphere-styles')) return; // idempotent — safe if multiple instances exist
   ...
   */
  update(state: AtmosphereState, deltaTime: number = 0): void {
  ...
  private triggerGlitchPulse(): void {
