    ...
    setGrid(grid) {
        ...
        for (let row = 0; row < grid.length; row++) {
            ...
            for (let col = 0; col < rowCells.length; col++) {
                ...
                if (!cell)
                    continue; // null = no tile here (hole/unrevealed), skip
 ...
 */
export async function loadTileSet(basePath, tileNames) {
    const loader = new THREE.TextureLoader();
    const entries = await Promise.all(tileNames.map((name) => new Promise((resolve, reject) => {
        loader.load(`${basePath}/${name}.png`, (tex) => {
            tex.colorSpace = THREE.SRGBColorSpace;
            tex.magFilter = THREE.LinearFilter;
            tex.minFilter = THREE.LinearMipmapLinearFilter;
            resolve([name, tex]);
        }, undefined, (err) => reject(err instanceof Error ? err : new Error(`Failed to load tile: ${name}`)));
    })));
    return Object.fromEntries(entries);
}
//# sourceMappingURL=IsoTileMap.js.map