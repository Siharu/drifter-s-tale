...
let weatherState: WeatherType = 'OVERCAST' as WeatherType;
let wrongnessState: WrongnessState = WrongnessState.GREY; // RS7, Day 9 — wrongness already accumulating
let fogIntensity = 0.35;

function applySkyState(): void {
...
const tRange = panel.querySelector<HTMLInputElement>('#tRange')!;
const tVal = panel.querySelector<HTMLSpanElement>('#tVal')!;
const wrSelect = panel.querySelector<HTMLSelectElement>('#wrSelect')!;
...
wrSelect.addEventListener('input', () => {
  wrongnessState = wrSelect.value as WrongnessState;
  applySkyState();
});
