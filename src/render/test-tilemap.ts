...
sky.update(0, { timeOfDay: 15, weatherState: 'CLEAR' as WeatherType, fogIntensity: 0.15 });
isoRenderer.syncSky();
...
async function main(): Promise<void> {
  ...
  for (let row = 0; row < size; row++) {
    const rowCells: { tileKey: string }[] = [];
    for (let col = 0; col < size; col++) {
      const r = rng();
      let tileKey = 'grass_a';
      if (r < 0.12) tileKey = 'dirt_a';
      else if (r < 0.2) tileKey = 'stone_basic';
      else if (r < 0.26) tileKey = 'stone_rocky_grey';
      else if (r < 0.32) tileKey = 'forest_a';
